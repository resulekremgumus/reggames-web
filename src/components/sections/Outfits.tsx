  ...
  return (
    <section className="px-[clamp(18px,5vw,52px)] py-[clamp(64px,9vw,112px)] max-w-[1160px] mx-auto">
      <Reveal className="text-center mb-2">
        <h2 className="font-heading font-extrabold text-[clamp(30px,4vw,52px)] tracking-[-0.02em]">Kostümler</h2>
      </Reveal>
      <Reveal className="text-center mb-10">
        <p className="text-text-muted text-sm">Seviye atla, yeni kostümlerin kilidini aç.</p>
      </Reveal>
        ...
        {outfits.map((o, i) => (
          <Reveal key={o.name} delay={i * 0.06}>
            <div className="relative rounded-2xl border overflow-hidden" style={{ background: "var(--color-card-2)", borderColor: "var(--color-border)" }}>
              <ImagePlaceholder label={`${o.name} kostüm (1:1)`} aspect="1/1" radius={0} className="w-full" />
              <span
